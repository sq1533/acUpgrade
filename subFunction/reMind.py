...
def reMind() -> None:
    ...
    if today.day not in toMonth:
        if datetime.now().strftime("%H:%M") == "11:00":
            read = pd.read_json("C:\\Users\\USER\\ve_1\\DB\\6reMind.json",orient='records',dtype={'date':str,'bank':str,'info':str,'signUp':str})
            if len(read.index.tolist()) == 1:pass
                ...
                for i in range(1,len(read.index.tolist())):
                    ...
                    time.sleep(1)
                #발송 후 데이터 리셋
                pd.DataFrame(data={"date":"test","bank":"test","info":"test","signUp":"test"},index=[0]).to_json("C:\\Users\\USER\\ve_1\\DB\\6reMind.json",orient='records',force_ascii=False,indent=4)
